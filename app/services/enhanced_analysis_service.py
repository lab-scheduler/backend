...
class EnhancedAnalysisService:
    ...
    def get_comprehensive_analysis(session, org_id: int, start: date, end: date):
        """Generate comprehensive analysis with full details"""
        ...
        report = {
            ...
            "executive_summary": EnhancedAnalysisService._generate_summary(shifts, assignments, departments),
            "departments": EnhancedAnalysisService._analyze_departments(departments, shifts, assignments, staff_list),
            "staff_details": EnhancedAnalysisService._analyze_staff(staff_list, assignments, shifts),
            "shifts": EnhancedAnalysisService._detail_shifts(shifts, assignments, departments, staff_list),
            "analytics": EnhancedAnalysisService._calculate_analytics(shifts, assignments, staff_list, departments),
            "recommendations": EnhancedAnalysisService._generate_recommendations(shifts, assignments, departments)
        }
    ...
    def _load_assignments(session, org_id: int, start: date, end: date) -> List[ShiftAssignment]:
        """Load shift assignments for date range and organization"""
        from sqlmodel import select
        from app.db.models import Shift, ShiftAssignment

        # First get shifts in range for this organization
        shifts = session.exec(
            select(Shift).where(
                Shift.org_id == org_id,
        ...
        # Then get assignments for those shifts
        if shift_ids:
            return session.exec(
